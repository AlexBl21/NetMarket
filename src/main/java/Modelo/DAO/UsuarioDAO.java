...
public class UsuarioDAO implements IUsuario {
    ...
    public Usuario consultarPorId(Usuario usuario) {
        ...
        try {
            connection = BaseDeDatos.getConnection();
            sentencia = connection.prepareStatement(SQL_CONSULTARID, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.TYPE_FORWARD_ONLY); //esto sirve para que prepare la tabla de datos(lo de resultset garantiza que devuelva un solo resultadoo)
            sentencia.setInt(1, usuario.getId());
            resultado = sentencia.executeQuery();//toma la forma del registro
